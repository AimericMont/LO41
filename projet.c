...
{
    //printf("Identified by the key : %d\n",semkey);
	int status = 0;		
	int semid_init;
   	union semun {
		int val;
...
int* remplir_alloc()//creer un seg de taille date soumission max plus n * tmp exec max //au passage
{
	...
	for (int i = 0; i < l; ++i)
	 ...
	 for (int i=0;i<l;i++)
	  {
	  	//e=rand()%nb;
	  	//ajouter_int_alloc(p_alloc,sum,p_desc[e*4+1]);
	  	e=(rand()%10)+1;
	  	p_alloc[i]=e;
	  }
	  printf("alloc table : ");
...
int start_process(int id)
{
	int k,v,prio;
		...
		/*printf("tourniquet at init state is : ");
		for(int i=0;i<nb;i++)
		{
			printf("%d ",p_trn[prio-1][i]);
		}
		printf("\n");*/
...
int fonc_roundRobin()
{
	...
	if(!(parent=fork()))//this is the schduler
	{
		...
		while(!check_process() /*&& p_alloc[0]!=NULL*/){
			//printf("hello\n");
			P(sem_id,0);
			check_start_process();
			print_trn();
			...
			//prio=p_desc[(elu_id-1)*4+1];
			//p_desc[(elu_id-1)*4+1]--;
			//p_desc[(elu_id-1)*4+3]--;
			//sortir_int(p_trn[prio],nb);
			p_quantum[0]++;
...
int main(int argc, char* argv[])
{
	...
	remplir_desc(nb);
	//printf("id of process pointed by p_desc is : %d\n",p_desc[0] );
	...
	/*printf("vals du groupe de sem_id_process :");
	for (int i = 0; i < nb; ++i)
	{
		printf("%d ", semctl(sem_id_process,i,GETVAL) );
	}
	printf("\n");*/
